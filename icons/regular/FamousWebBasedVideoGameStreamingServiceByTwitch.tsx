/**
 * Original name: Famous web based video game streaming service by twitch Icon Free Download
 */
export function FamousWebBasedVideoGameStreamingServiceByTwitch({
  size = 24,
}: {
  size?: number;
}): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      enable-background="new 0 0 24 24"
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="m9.444 24h-3.07c-.414 0-.75-.336-.75-.75v-2.186h-4.874c-.414 0-.75-.336-.75-.75v-15.65c0-.094.018-.186.052-.273l1.533-3.914c.112-.288.39-.477.698-.477h20.967c.414 0 .75.336.75.75v13.697c0 .205-.084.401-.231.542l-6.138 5.867c-.14.134-.325.208-.519.208h-4.302l-2.848 2.728c-.139.133-.324.208-.518.208zm-2.32-1.5h2.02l2.848-2.728c.14-.134.325-.208.519-.208h4.302l5.688-5.438v-12.626h-19.706l-1.295 3.306v14.759h4.874c.414 0 .75.336.75.75z" />
      <path d="m9.555 20.087c-.101 0-.201-.02-.297-.062-.275-.119-.453-.389-.453-.688v-2.098h-5.12c-.414 0-.75-.336-.75-.75v-13.782c0-.414.336-.75.75-.75h16.631c.414 0 .75.336.75.75v10.456c0 .207-.085.404-.235.546l-3.528 3.327c-.14.131-.323.204-.515.204h-3.914l-2.804 2.644c-.142.133-.328.203-.515.203zm-5.12-4.348h5.12c.414 0 .75.336.75.75v1.11l1.756-1.656c.14-.131.323-.204.515-.204h3.914l3.076-2.9v-9.382h-15.131z" />
      <path d="m16.75 12c-.414 0-.75-.336-.75-.75v-4.5c0-.414.336-.75.75-.75s.75.336.75.75v4.5c0 .414-.336.75-.75.75z" />
      <path d="m11.782 12c-.414 0-.75-.336-.75-.75v-4.5c0-.414.336-.75.75-.75s.75.336.75.75v4.5c0 .414-.336.75-.75.75z" />
    </svg>
  );
}
